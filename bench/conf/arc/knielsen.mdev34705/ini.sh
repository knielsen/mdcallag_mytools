...
x=0
while ! bin/mariadb -u${USER} -e 'select User, Host from mysql.user' ; do
  x=$(( x + 1 ))
  sleep 1
...
hn=$( hostname )
bin/mariadb -u${USER} -e "drop user ''@\"${hn}\""
bin/mariadb -u${USER} -e "drop user ''@localhost"
bin/mariadb -u${USER} -e "drop user root@localhost"
bin/mariadb -u${USER} -e "create user root@'%' identified by 'pw'"
bin/mariadb -u${USER} -e "grant all on *.* to root@'%' with grant option"
bin/mariadb -u${USER} -e 'select User, Host from mysql.user'
